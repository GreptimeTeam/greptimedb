...
| `region_engine.mito.experimental_series_index_max_size` | String | `5GiB` | Maximum local disk space for series indexes, range indexes, catalogs, and temporary output.<br/>Shared across all regions and workers. Older data is evicted first; builds defer while<br/>readers retain evicted files. Accounted in KiB units, excluding filesystem overhead.<br/>Minimum: 1KiB. Takes effect on restart. |
