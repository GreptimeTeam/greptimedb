...
| `meta_schema_name` | String | `greptime_schema` | Optional: The schema for metadata table and election table name.<br/>In PostgreSQL 15 and later, the default public schema is restricted by default,<br/>and non-superusers may no longer have CREATE privileges in the public schema.<br/>When encountering permission restrictions, use this parameter to specify a writable schema.<br/>**Only used when backend is `postgres_store`.** |
