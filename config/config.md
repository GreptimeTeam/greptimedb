...
| `region_engine.mito.experimental_series_index_max_size` | String | `5GiB` | Maximum installed series and range index bytes, shared across all regions and workers.<br/>Reconciliation evicts files with the oldest minimum timestamps first. Temporary output,<br/>catalogs, and retired files retained by readers are excluded; disk usage can exceed this limit.<br/>Minimum: 1KiB. Takes effect on restart. |
