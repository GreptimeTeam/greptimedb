...
| `workload_types` | Array | -- | The acceptable workload types of the datanode.<br/>The default value is `["hybrid"]`.<br/><br/>Available values(enterprise version):<br/>- `hybrid`: all workloads, which means the datanode can handle all regions.<br/>- `ingest`: only ingest workload, which means the datanode can only handle writable regions.<br/>- `query`: only query workload, which means the datanode can only handle read-only regions.<br/><br/>Available values(community version):<br/>- `hybrid`: all workloads, which means the datanode can handle all regions. |
