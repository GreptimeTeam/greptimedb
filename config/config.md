...
| `region_engine.mito.max_concurrent_scan_files` | Integer | `384` | Maximum number of SST files to scan concurrently when estimated memory protection<br/>is disabled or selected SSTs lack row-group size metadata. |
...
| `region_engine.mito.experimental_scan_memory_budget` | String | `25%` | Estimated SST decoding memory budget shared across queries and regions.<br/>Defaults to 25% of cgroup memory, or host memory when no cgroup limit is available.<br/>Supports absolute sizes (e.g., "2GB") or percentages. Exhaustion fails immediately.<br/>Independent of scan_memory_limit and scan_memory_on_exhausted; this is an estimate,<br/>not a limit on total process memory. Missing SST metadata uses existing file-count checks.<br/>Setting 0 or "unlimited" restores existing file-count checks for all scans. |
