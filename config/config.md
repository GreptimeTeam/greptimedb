...
| `region_engine.mito.enable_refill_cache_on_read` | Bool | `true` | Enable background downloading of files to the local cache when accessed during queries (default: true).<br/>When enabled, files will be asynchronously downloaded to improve performance for subsequent reads. |
