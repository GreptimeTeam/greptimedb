...
| `auto_create_table` | Bool | `true` | Server-side global switch for auto table creation on write.<br/>When `false`, a missing user table is never auto-created even if the request sets the `auto_create_table` hint to `true`. The internal events table may still be created or reconciled. Default: `true`. |
