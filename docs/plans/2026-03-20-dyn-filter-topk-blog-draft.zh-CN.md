...
| `ORDER BY start_time DESC LIMIT 10` | `0.334s` / `0.336s` / `0.342s` | `0.336s` / `0.340s` / `0.336s` | `start_time` 是时间索引列；window sort 在这类场景里本来就已经很快，所以切到 dyn filter 后变化不大 |

这里主要有两点：

- `end_time` 这类非时间索引排序查询，之前慢，主要是因为旧路径会扫描全表；切到 dyn filter 之后，TopK 的边界可以直接拿来提前剪掉后续扫描。
- `start_time` 和 `end_time` 这两类 TopK 查询，现在都走 dyn filter；window sort 那条更复杂、也更依赖 time index 数据分布的路径，已经被这条更统一的执行方式取代。
