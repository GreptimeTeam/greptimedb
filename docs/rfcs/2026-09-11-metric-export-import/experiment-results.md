...
These experiments support three changes: physical-group export, batched logical
...
| Physical tag columns | Execution order | Ordinary COPY (ms) | Physical-group export (ms) | Speedup |
| ---: | --- | ---: | ---: | ---: |
...
Physical-group export was **2.82–3.39x faster** in these runs. Each populated
...
The measured single-region query used an ordered streaming merge without a
separate SortExec. Multi-region behavior and broader Metric column types remain
implementation acceptance work.

## Batched logical DDL

The restore fixture has one physical group, one file per logical table, two rows
per populated table and one empty table. The 10,000-table case contains 19,998
rows. The three paths use SQL HTTP per table, an internal call per table, and
internal batches of up to 128 tables. The internal single-call control separates
transport savings from procedure batching.
...
Batch-DDL checks covered two schemas, ordinary tables/views, failure after a
successful batch and retry. Existing standalone COPY cases also passed.
