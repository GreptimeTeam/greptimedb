...
-- planner's counterpart of Prometheus' `StepInvariantExpr`. The window `[0s, 300s]` holds the
-- counter going from 0 to 300 for host 'a' (1.0 per second) and from 100 to 250 for host 'b'
-- (0.5 per second); every step must report those same per-series values, and both series must be
-- there at every step.
-- SQLNESS SORT_RESULT 3 1
...
-- a regression read at the wrong instant predicts 60s ahead of the window's last sample instead,
-- i.e. 5.0 at every step — the sample at 240s is 4.0 and the trend rises by 1.0 per minute — and
-- shows up as a constant here.
-- SQLNESS SORT_RESULT 3 1
