...
INSERT INTO vector_overfetch VALUES
...
DROP TABLE vector_overfetch;

DROP TABLE IF EXISTS vector_overfetch_partitioned_hard;

CREATE TABLE vector_overfetch_partitioned_hard (
  ts TIMESTAMP TIME INDEX,
  "id" INT PRIMARY KEY,
  "tag" STRING,
  v VECTOR(2) NULL VECTOR INDEX WITH(engine = 'usearch', metric = 'l2sq')
...
INSERT INTO vector_overfetch_partitioned_hard VALUES
...
SELECT
  ...
  "tag",
  vec_l2sq_distance(v, '[0.0, 0.0]') AS d
FROM vector_overfetch_partitioned_hard
...
DROP TABLE vector_overfetch_partitioned_hard;
