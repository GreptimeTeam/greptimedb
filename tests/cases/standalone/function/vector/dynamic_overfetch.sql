...
CREATE TABLE vector_overfetch_sa (
...
INSERT INTO vector_overfetch_sa VALUES
...
ADMIN FLUSH_TABLE('vector_overfetch_sa');

SELECT
  "id",
  "tag",
  vec_l2sq_distance(v, '[0.0, 0.0]') AS d
FROM vector_overfetch_sa
WHERE "tag" = 'odd'
...
DROP TABLE vector_overfetch_sa;

-- Part 2: Multi-region (cross-partition filter + tie-breaker + high offset)

CREATE TABLE vector_overfetch_partitioned_sa (
  ts TIMESTAMP TIME INDEX,
  "id" INT PRIMARY KEY,
  "tag" STRING,
  v VECTOR(2) NOT NULL VECTOR INDEX WITH(engine = 'usearch', metric = 'l2sq')
...
INSERT INTO vector_overfetch_partitioned_sa VALUES
...
ADMIN FLUSH_TABLE('vector_overfetch_partitioned_sa');

SELECT
  "id",
  "tag",
  vec_l2sq_distance(v, '[0.0, 0.0]') AS d
FROM vector_overfetch_partitioned_sa
...
DROP TABLE vector_overfetch_partitioned_sa;
