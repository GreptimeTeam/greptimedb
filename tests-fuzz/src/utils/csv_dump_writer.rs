...
use crate::utils::{get_gt_fuzz_dump_dir, get_gt_fuzz_dump_suffix};
...
impl CsvDumpSession {
    ...
    pub fn append(&mut self, records: CsvRecords) {
    ...
    pub fn flush_all(&self) -> Result<()> {
        for batch in &self.records {
            write_batch_csv(&self.run_dir, batch)?;
        }
        Ok(())
    }
}
...
fn write_batch_csv(run_dir: &Path, batch: &CsvRecords) -> Result<()> {
...
mod tests {
    ...
    fn test_create_session_and_flush() {
        let mut session =
            CsvDumpSession::new(CsvDumpMetadata::new("fuzz_case", 1, 2, 3, 4)).unwrap();
        session.append(CsvRecords {
