...
| Overall Request P99 | `histogram_quantile(0.99, sum by (le) (rate(greptime_servers_http_requests_elapsed_bucket{instance=~"$frontend",path!~"/health\|/metrics"}[$__rate_interval])))` | `timeseries` | Tail latency for HTTP requests served by frontends, excluding health and metrics endpoints. | `prometheus` | `s` | `http-p99` |
# Incident triage
| Title | Query | Type | Description | Datasource | Unit | Legend Format |
| --- | --- | --- | --- | --- | --- | --- |
