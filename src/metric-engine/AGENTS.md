...
are multiplexed by injecting `__table_id` and `__tsid` columns, so reads filter
back out the rows for one logical table. It implements `RegionEngine` and
delegates all real storage to `mito2`.
...
| `row_modifier` | `src/metric-engine/src/row_modifier.rs` | Adds `__tsid` (hash of tags) and `__table_id` to incoming rows |
| `batch_modifier` | `src/metric-engine/src/batch_modifier.rs` | RecordBatch-level `__tsid` computation |
...
columns from `metadata_region`, then `row_modifier` injects `__tsid`/`__table_id`
and the request is forwarded to the Mito2 data region.
...
- **Reserved column ids / names** (`__tsid`, `__table_id`): see
...
- `__tsid` must be stable for the same tag set — it is a hash over sorted tag
  names + values.
