    ...
    T: Scalar<RefType<'a> = T>,
{
    ...
    fn try_create(vector: &'a VectorRef) -> Result<Self> {
        ...
        let col: &PrimitiveVector<T> = Helper::check_get(inner)?;
...
impl<'a> ScalarViewer<'a> for BooleanViewer {
    ...
    fn try_create(vector: &VectorRef) -> Result<Self> {
        ...
        let (inner, validity) = try_extract_inner(vector)?;
        let col: &BooleanVector = Helper::check_get(inner)?;
...
impl<'a> ScalarViewer<'a> for StringViewer<'a> {
    ...
    fn try_create(vector: &'a VectorRef) -> Result<Self> {
        let (inner, validity) = try_extract_inner(vector)?;
        let col: &'a StringVector = Helper::check_get(inner)?;
...
where T: Scalar<Viewer<'a> = Self> + ObjectType
{
    ...
    fn try_create(vector: &'a VectorRef) -> Result<Self> {
        let (inner, validity) = try_extract_inner(vector)?;
        let col: &ObjectVector<T> = Helper::check_get(inner)?;
        let values = col.values();
...
impl<'a> ScalarViewer<'a> for ArrayViewer<'a> {
    ...
    fn try_create(vector: &'a VectorRef) -> Result<Self> {
        let (inner, validity) = try_extract_inner(vector)?;
        let col: &'a ArrayVector = Helper::check_get(inner)?;
...
fn try_extract_inner(vector: &VectorRef) -> Result<(&VectorRef, Bitmap)> {
    ...
    let (vector, validity) = if vector.is_const() {
        ...
        let c: &ConstVector = unsafe { Helper::static_cast(vector) };
        (c.inner(), bitmap.into())
    } else if vector.is_nullable() {
        let c: &NullableVector = unsafe { Helper::static_cast(vector) };
    ...
    let vector: &VectorRef = if vector.is_const() {
        let vector: &ConstVector = unsafe { Helper::static_cast(vector) };
        vector.inner()
    } else if vector.is_nullable() {
        let vector: &NullableVector = unsafe { Helper::static_cast(vector) };
        vector.inner()
    } else {
        vector
    };
