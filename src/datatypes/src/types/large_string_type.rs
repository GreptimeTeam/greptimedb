...
use crate::vectors::{LargeStringVectorBuilder, MutableVector};

#[derive(Debug, Default, Clone, PartialEq, Eq, Hash, PartialOrd, Ord, Serialize, Deserialize)]
pub struct LargeStringType;

impl LargeStringType {
    pub fn arc() -> DataTypeRef {
        Arc::new(Self)
    }
}

impl DataType for LargeStringType {
    fn name(&self) -> String {
        "LargeString".to_string()
    }

    fn logical_type_id(&self) -> LogicalTypeId {
        LogicalTypeId::LargeString
    }
    ...
    fn create_mutable_vector(&self, capacity: usize) -> Box<dyn MutableVector> {
        Box::new(LargeStringVectorBuilder::with_capacity(capacity))
    }
