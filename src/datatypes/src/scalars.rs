...
/// A wrapper type for large strings to distinguish from regular strings in the type system
...
impl From<&str> for LargeString {
    fn from(s: &str) -> Self {
        LargeString(s.to_string())
    }
}

impl AsRef<str> for LargeString {
    fn as_ref(&self) -> &str {
        &self.0
    }
}

impl std::ops::Deref for LargeString {
    type Target = str;
...
/// A reference wrapper for large strings
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, PartialOrd, Ord, serde::Serialize)]
pub struct LargeStringRef<'a>(pub &'a str);

impl<'a> AsRef<str> for LargeStringRef<'a> {
    fn as_ref(&self) -> &str {
        self.0
    }
}

impl<'a> std::ops::Deref for LargeStringRef<'a> {
    type Target = str;
...
impl<'a> ScalarRef<'a> for LargeStringRef<'a> {
    type ScalarType = LargeString;

    fn to_owned_scalar(&self) -> LargeString {
        LargeString(self.0.to_string())
    }
}

impl Scalar for LargeString {
    type VectorType = crate::vectors::LargeStringVector;
    type RefType<'a> = LargeStringRef<'a>;

    fn as_scalar_ref(&self) -> LargeStringRef<'_> {
        LargeStringRef(&self.0)
    }

    fn upcast_gat<'short, 'long: 'short>(long: LargeStringRef<'long>) -> LargeStringRef<'short> {
        LargeStringRef(long.0)
    }
}
