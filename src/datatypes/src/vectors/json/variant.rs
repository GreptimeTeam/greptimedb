...
pub(super) fn append_json_variant(
    ...
    match value {
        ...
        JsonVariant::Number(JsonNumber::Float(value)) if value.0.is_finite() => {
            builder.append_value(value.0)
        }
        JsonVariant::Number(JsonNumber::Float(_)) => builder.append_value("NaN"),
...
fn append_u64(
    ...
    let value = VariantDecimal16::try_new(value as i128, 0).map_err(|e| {
        ArrowError::InvalidArgumentError(format!(
            "Failed to encode JSON integer as Variant Decimal16: {e}"
        ))
    })?;
    builder.append_value(value);
    Ok(())
