...
use crate::ddl::create_flow::{DEFER_ON_MISSING_SOURCE_KEY, FlowType};
use crate::ddl::test_util::create_table::test_create_table_task;
use crate::ddl::tests::create_flow::create_test_pending_flow;
...
#[tokio::test]
async fn test_activate_pending_batching_flow_with_instant_ttl_source() {
    let source_table_names = vec![TableName::new(
        DEFAULT_CATALOG_NAME,
        DEFAULT_SCHEMA_NAME,
    ...
    assert!(activated_flow.is_active());
    assert_eq!(activated_flow.unresolved_source_table_names().len(), 0);
    assert_eq!(activated_flow.source_table_ids(), &[1025]);
    assert_eq!(activated_flow.last_activation_error(), &None);
}
...
async fn test_activate_pending_flow_preserves_streaming_type() {
    ...
    assert!(!create_requests.is_empty());
    for req in create_requests.iter() {
        assert!(!req.flow_options.contains_key(DEFER_ON_MISSING_SOURCE_KEY));
        assert_eq!(
            req.flow_options
                .get(FlowType::FLOW_TYPE_KEY)
                .map(String::as_str),
