            ...
            b.and_then(|b| {
                ...
                for (idx,field) in projected_schema.fields.iter().enumerate() {
                    ...
                    if column.data_type() != field.data_type() {
                        let logical_type: Option<&str> = field.metadata().get("greptime:type").map(|s| s.as_str());
                        let output = custom_cast(&column, field.data_type(), logical_type)?;
                        columns.push(output)
                    } else {
...
fn custom_cast(
    ...
    if let ArrowDataType::Map(_, _) = array.data_type() {
        if let ArrowDataType::Binary = target_type {
            return convert_map_to_json_binary(array, logical_type);
        }
    }
...
fn convert_map_to_json_binary(
    ...
    for i in 0..map_array.len() {
        if map_array.is_null(i) {
            ...
            let json_bytes = if let Some("Json") = logical_type {
...
mod test {
    ...
    fn test_convert_map_to_json_binary() {
        ...
        let result_json = convert_map_to_json_binary(&map_array, Some("Json")).unwrap();
