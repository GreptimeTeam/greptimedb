...
pub(crate) struct ProcedureMeta {
    ...
    /// The internal event emitted by the procedure. It's generated by [Procedure::event].
    internal_event: Option<Arc<dyn Event>>,
}
...
impl ProcedureMeta {
    ...
    fn set_state(&self, state: ProcedureState) {
        if let Some(event_recorder) = &self.event_recorder {
            if let Some(internal_event) = &self.internal_event {
...
impl LocalManager {
    ...
    fn submit_root(
        ...
        let internal_event = procedure.event().clone();
