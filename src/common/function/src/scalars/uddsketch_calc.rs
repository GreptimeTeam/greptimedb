...
impl Function for UddSketchMergeStateFunction {
    ...
    fn invoke_with_args(
        ...
        // The `uddsketch_state` accumulator takes a u32 bucket size; the
        // scalar receives an Int64 literal from the rewritten flow plan, so
        // convert it with a checked conversion now that the per-row validation
        // above guarantees a single constant value.
        let bucket_size = u32::try_from(bucket_size).map_err(|_| {
            DataFusionError::Execution(format!(
                "'{}' expects bucket size to fit in u32, got {bucket_size}",
                self.name()
            ))
        })?;
...
mod tests {
    ...
    fn make_state(bucket_size: u32, error_rate: f64, values: &[f64]) -> Vec<u8> {
    ...
    fn test_uddsketch_merge_state_function() {
        ...
        assert_eq!(result.len(), 1);
        let merged = UddSketchRef::parse(result.value(0)).unwrap();
    ...
    fn test_uddsketch_merge_state_function_rejects_parameter_mismatch() {
        ...
        let function = UddSketchMergeStateFunction::default();
        let args = vec![
            ColumnarValue::Array(Arc::new(Int64Array::from(vec![20]))),
            ColumnarValue::Array(Arc::new(Float64Array::from(vec![0.01]))),
            ColumnarValue::Array(Arc::new(BinaryArray::from_opt_vec(vec![Some(
                state1_bytes.as_slice(),
            )]))),
    ...
    #[test]
    fn test_uddsketch_calc_function_out_of_range_percentile_is_null() {
        let function = UddSketchCalcFunction::default();
        let state = make_state(10, 0.01, &[1.0, 2.0]);

        // Out-of-range percentiles are rejected by the underlying quantile
        // computation and surface as NULL, never a panic or a bogus value.
        for percentile in [-0.1f64, 1.1] {
            ...
            assert!(
                result.is_null(0),
                "percentile {percentile} must yield NULL instead of panicking"
            );
        }
    }
    ...
    fn test_uddsketch_merge_state_function_null_validates_present_state() {
        ...
        for (delta, sink) in [
            ...
            let expected = UddSketchRef::parse(expected.value(0)).unwrap();
            assert_eq!(expected.count(), merged.count());
            assert!((expected.sum() - merged.sum()).abs() < 1e-10);
            for q in [0.1, 0.5, 0.9] {
                let expected_q = expected.quantile(q).unwrap().unwrap();
                let merged_q = merged.quantile(q).unwrap().unwrap();
                assert!(
                    (expected_q - merged_q).abs() < 1e-10,
                    "quantile {q} mismatch: expected={expected_q}, got={merged_q}"
                );
            }
        }
    }
