...
pub trait ExecutionPlan: Debug + Send + Sync {
    ...
    /// Different with DataFusion's `ExecutionPlan::execute` in that the argument `runtime` is
    /// optional, making caller's life easier when runtime is not used.
    async fn execute(
        &self,
        partition: usize,
...
pub struct ExecutionPlanAdapter {
    pub inner: Arc<dyn DfExecutionPlan>,
}
...
impl ExecutionPlan for ExecutionPlanAdapter {
    ...
    fn schema(&self) -> SchemaRef {
        ...
        Arc::new(self.inner.schema().try_into().unwrap())
    }
    ...
    fn children(&self) -> Vec<Arc<dyn ExecutionPlan>> {
        self.inner
            .children()
            .into_iter()
            .map(|x| Arc::new(ExecutionPlanAdapter { inner: x }) as _)
            .collect()
    }
    ...
    fn with_new_children(
        ...
        let children = children
            .into_iter()
            .map(|x| Arc::new(DfExecutionPlanAdapter { inner: x }) as _)
            .collect();
        let plan = self
            .inner
            .with_new_children(children)
            .context(error::GeneralDataFusionSnafu)?;
        Ok(Arc::new(ExecutionPlanAdapter { inner: plan }))
    }
    ...
    async fn execute(
        ...
        let runtime = runtime.context(error::MissingRuntimeSnafu)?;
        let stream = self
            .inner
...
#[derive(Debug)]
pub struct DfExecutionPlanAdapter {
...
impl DfExecutionPlan for DfExecutionPlanAdapter {
    ...
    fn with_new_children(
        ...
        let children = children
            ...
            .map(|x| Arc::new(ExecutionPlanAdapter { inner: x }) as _)
            .collect();
        let plan = self.inner.with_new_children(children)?;
        Ok(Arc::new(DfExecutionPlanAdapter { inner: plan }))
    }
    ...
    async fn execute(
        ...
        let stream = self.inner.execute(partition, Some(runtime)).await?;
