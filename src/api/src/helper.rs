...
///
///
/// For example: we can use `ColumnDataTypeWrapper::timestamp_second_datatype()`,
/// to get a ColumnDataTypeWrapper with datatype `ColumnDataType::TimestampSecond`.
...
pub fn to_proto_value(value: Value) -> Option<v1::Value> {
    let proto_value = match value {
        ...
        Value::Duration(_) | Value::List(_) => return None,
    };
...
pub fn value_to_grpc_value(value: Value) -> GrpcValue {
    GrpcValue {
        value_data: match value {
            ...
            Value::Duration(_) | Value::List(_) => unreachable!(),
        },
    }
}
