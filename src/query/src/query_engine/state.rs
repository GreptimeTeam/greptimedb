...
use crate::optimizer::promql_series_aggregate::PromqlSeriesAggregate;
...
impl QueryEngineState {
    ...
    pub fn try_new(
        ...
        if options.enable_promql_series_aggregate {
            physical_optimizer
                .rules
                .push(Arc::new(PromqlSeriesAggregate));
        }
...
mod tests {
    ...
    #[test]
    fn promql_series_aggregate_rule_follows_its_switch() {
        for enabled in [true, false] {
            let state = new_query_engine_state_with(
                Plugins::default(),
                QueryOptions {
                    enable_promql_series_aggregate: enabled,
                    ..Default::default()
                },
            );
            let installed = state
                ...
                .physical_optimizers()
                .iter()
                .any(|rule| rule.name() == PromqlSeriesAggregate.name());
