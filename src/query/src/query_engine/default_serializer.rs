...
mod tests {
  ...
  TableScan: devices",
    ...
    /// which breaks the plan as soon as the two tables don't share their columns (the root cause of
    /// the nested broadcast join bug of the two datanodes, see
    /// `tests-integration/src/tests/nested_broadcast_join_test.rs`).
    #[tokio::test]
