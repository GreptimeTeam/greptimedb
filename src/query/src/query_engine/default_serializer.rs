...
mod tests {
  ...
  TableScan: devices",
    ...
    #[tokio::test]
    async fn test_serializer_preserves_sort_fetch() {
        let catalog_list = catalog::memory::new_memory_catalog_manager().unwrap();
        let factory = QueryEngineFactory::new(
            catalog_list,
            None,
        ...
        let table_provider = Arc::new(mock_table_provider(1.into()));
        let sorted = LogicalPlanBuilder::scan(
            "devices",
            Arc::new(LogicalTableSource::new(table_provider.schema().clone())),
            None,
        )
        .unwrap()
        .sort(vec![col("k0").sort(true, false)])
        .unwrap()
        .build()
        .unwrap();
        let LogicalPlan::Sort(mut sort) = sorted else {
        ...
        let plan_str = decode_plan.to_string();
        match &decode_plan {
            LogicalPlan::Sort(sort) => assert_eq!(sort.fetch, Some(7)),
            LogicalPlan::Limit(_) => {
                assert!(
                    plan_str.contains("fetch=7"),
                    "decoded plan should preserve fetch=7, got: {plan_str}"
                );
            }
            other => panic!("unexpected decoded plan: {other}"),
        }
    }
}
