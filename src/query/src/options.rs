...
/// Parse the scheduled runtime (in milliseconds since Unix epoch) from extensions.
///
/// Returns `Ok(None)` if the extension key is absent, `Ok(Some(millis))` on success,
/// or `Err` if the value is malformed.
pub fn parse_scheduled_runtime_millis(extensions: &HashMap<String, String>) -> Result<Option<i64>> {
    match extensions.get(FLOW_SCHEDULED_RUNTIME_MILLIS) {
        Some(val) => val.parse::<i64>().map(Some).map_err(|_| {
            invalid_query_context_extension(format!(
                "Invalid value for {}: {}",
                FLOW_SCHEDULED_RUNTIME_MILLIS, val
            ))
        }),
        None => Ok(None),
    }
...
/// Parse the scheduled runtime from extensions into a [`DateTime<Utc>`].
///
...
/// also rejected.
pub fn parse_scheduled_runtime_datetime(
    extensions: &HashMap<String, String>,
) -> Result<Option<DateTime<Utc>>> {
    match parse_scheduled_runtime_millis(extensions)? {
        Some(millis) => DateTime::from_timestamp_millis(millis)
            .map(Some)
            .ok_or_else(|| {
                invalid_query_context_extension(format!(
                    "Out-of-range timestamp for {}: {} ms",
                    FLOW_SCHEDULED_RUNTIME_MILLIS, millis
                ))
            }),
        None => Ok(None),
    }
...
/// Convenience helper: extract scheduled runtime from a [`QueryContextRef`] as a [`DateTime<Utc>`].
///
/// Errors are silently swallowed, returning `None`.
/// Use [`parse_scheduled_runtime_datetime`] when a `Result` is needed.
pub fn scheduled_runtime_from_ctx(query_ctx: &QueryContextRef) -> Option<DateTime<Utc>> {
    let extensions = query_ctx.extensions();
    parse_scheduled_runtime_datetime(&extensions).ok().flatten()
}
...
mod flow_extension_tests {
    ...
    // --- scheduled runtime helper tests ---

    #[test]
    fn test_parse_scheduled_runtime_millis_absent() {
        let exts = HashMap::new();
        assert_eq!(parse_scheduled_runtime_millis(&exts).unwrap(), None);
    }

    #[test]
    fn test_parse_scheduled_runtime_millis_valid() {
        let exts = HashMap::from([(
            FLOW_SCHEDULED_RUNTIME_MILLIS.to_string(),
            "1700000000000".to_string(),
        )]);
        assert_eq!(
            parse_scheduled_runtime_millis(&exts).unwrap(),
            Some(1700000000000)
        );
    }

    #[test]
    fn test_parse_scheduled_runtime_millis_malformed() {
        let exts = HashMap::from([(
            FLOW_SCHEDULED_RUNTIME_MILLIS.to_string(),
            "not-a-number".to_string(),
        )]);
        let err = parse_scheduled_runtime_millis(&exts).unwrap_err();
        assert!(format!("{err}").contains(FLOW_SCHEDULED_RUNTIME_MILLIS));
    }

    #[test]
    fn test_parse_scheduled_runtime_datetime_valid() {
        let exts = HashMap::from([(
            FLOW_SCHEDULED_RUNTIME_MILLIS.to_string(),
            "1700000000000".to_string(),
        )]);
        let dt = parse_scheduled_runtime_datetime(&exts).unwrap().unwrap();
        assert_eq!(dt.timestamp_millis(), 1700000000000);
    }

    #[test]
    fn test_parse_scheduled_runtime_datetime_negative_millis() {
        let exts = HashMap::from([(FLOW_SCHEDULED_RUNTIME_MILLIS.to_string(), "-1".to_string())]);
        let dt = parse_scheduled_runtime_datetime(&exts).unwrap().unwrap();
        assert_eq!(dt.timestamp_millis(), -1);
    }

    #[test]
    fn test_parse_scheduled_runtime_datetime_out_of_range() {
        // i64::MAX millis is well beyond representable chrono::DateTime range
        let exts = HashMap::from([(
            FLOW_SCHEDULED_RUNTIME_MILLIS.to_string(),
            i64::MAX.to_string(),
        )]);
        let err = parse_scheduled_runtime_datetime(&exts).unwrap_err();
