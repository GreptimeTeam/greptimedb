...
impl PassDistribution {
    ...
    fn rewrite_with_distribution(
        ...
        for (idx, child) in children.into_iter().enumerate() {
            let child_req =
                Self::child_distribution_requirement(plan.as_ref(), idx, &current_req, &required);
    ...
    fn child_distribution_requirement(
        ...
        match required.get(child_idx) {
            Some(Distribution::UnspecifiedDistribution) if child_idx == 0 => {
                Self::map_hash_requirement_through_projection(plan, current_req)
            }
