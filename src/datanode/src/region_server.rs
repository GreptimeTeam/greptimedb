...
mod tests {
    ...
    #[tokio::test]
    async fn test_failed_insert_count_status() {
        let failed_insert_count = &*crate::metrics::REGION_SERVER_INSERT_FAIL_COUNT;
        let put_request = || {
        ...
        for (status, status_label) in [
            (StatusCode::RegionNotFound, "RegionNotFound"),
            (StatusCode::RegionNotReady, "RegionNotReady"),
            (StatusCode::RegionBusy, "RegionBusy"),
            (StatusCode::Internal, "Internal"),
        ] {
            let mut server = mock_region_server();
            let (engine, _receiver) = MockRegionEngine::with_mock_fn(
                MITO_ENGINE_NAME,
                Box::new(move |region_id, _request| {
                    if region_id.region_number() == 2 {
                        return Ok(3);
                    }
                    let err = match status {
                        StatusCode::RegionNotFound => RegionNotFoundSnafu { region_id }.build(),
                        StatusCode::RegionNotReady => RegionNotReadySnafu { region_id }.build(),
                        StatusCode::RegionBusy => error::RegionBusySnafu { region_id }.build(),
                        StatusCode::Internal => error::MissingCacheSnafu.build(),
                        _ => unreachable!(),
                    };
                    Err(err)
                }),
            );
            server.register_engine(engine.clone());
            let failed_region_id = RegionId::new(1, 1);
            let successful_region_id = RegionId::new(1, 2);
            for region_id in [failed_region_id, successful_region_id] {
                server
                    .inner
                    .region_map
                    .insert(region_id, RegionEngineWithStatus::Ready(engine.clone()));
            ...
            for (request, successful_request) in [
                (put_request(), put_request()),
                (delete_request(), delete_request()),
            ] {
                let counter =
                    failed_insert_count.with_label_values(&[request.request_type(), status_label]);
                let before = counter.get();
                let response = server
                    .handle_request(successful_region_id, successful_request)
                    .await
                    .unwrap();
                ...
                let err = server
                    .handle_request(failed_region_id, request)
                    .await
                    .unwrap_err();
                ...
                let compact_counter =
                    failed_insert_count.with_label_values(&["Compact", status_label]);
                let before_compact = compact_counter.get();
                let err = server
                    .handle_request(
                        failed_region_id,
                ...
                assert_eq!(err.status_code(), status);
                assert_eq!(counter.get(), before + 1);
                assert_eq!(compact_counter.get(), before_compact);
            }
        }

        // Keep both paths in one test so their shared Put/RegionNotFound counter
        // cannot race between tests. Use the real metric engine to exercise batching.
        let mut engine_env = TestEnv::with_prefix("failed-insert-status").await;
        let mito_engine = engine_env.create_engine(Default::default()).await;
        let metric_engine =
            Arc::new(MetricEngine::try_new(mito_engine.clone(), Default::default()).unwrap());
        let mut server = mock_region_server();
        server.register_engine(metric_engine.clone());
        let region_id = RegionId::new(2, 1);
        // The server routes to the engine, but the logical region is absent there.
        server
            .inner
            .region_map
            .insert(region_id, RegionEngineWithStatus::Ready(metric_engine));
        let counter = failed_insert_count.with_label_values(&["Put", "RegionNotFound"]);
        let before = counter.get();
        let err = server
            .try_handle_metric_batch_puts(vec![
                (region_id, put_request()),
                (RegionId::new(2, 2), put_request()),
                (RegionId::new(2, 3), put_request()),
            ])
            .await
            .unwrap_err();
        ...
        for family in failed_insert_count.collect() {
            for metric in family.get_metric() {
                let labels = metric.get_label();
                assert_eq!(labels.len(), 2);
                assert_eq!(labels[0].name(), crate::metrics::REGION_REQUEST_TYPE);
                assert_eq!(labels[1].name(), "status");
            }
        }
        server.stop().await.unwrap();
        mito_engine.stop().await.unwrap();
    }
