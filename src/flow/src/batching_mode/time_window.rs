...
fn binary_search_expr(
    ...
    df_schema: &DFSchema,
        ...
        UnexpectedSnafu {
            reason: format!(
                " unit mismatch for time window expression {phy_expr:?}, found {lower_bound:?} and {upper_bound:?}"
            ),
        }
    );
...
mod test {
    ...
    use super::*;
    use crate::batching_mode::utils::{AddFilterRewriter, df_plan_to_sql, sql_to_df_plan};
