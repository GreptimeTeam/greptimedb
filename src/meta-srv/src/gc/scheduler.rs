...
impl GcScheduler {
    ...
    pub(crate) async fn handle_tick(&self) -> Result<GcJobReport> {
        ...
        let report = self
            .trigger_gc(PersistentEventContext::new(TriggerReason::ScheduledGc))
            .instrument(span)
            .await?;
    ...
    pub(crate) async fn handle_manual_gc(
        ...
        let Some(regions) = region_ids else {
            let report = self
                .trigger_gc(PersistentEventContext::new(TriggerReason::Manual))
                .await?;
...
mod tests {
    ...
    impl SchedulerCtx for CountingSchedulerCtx {
        ...
        async fn gc_regions(
            ...
            _event_context: PersistentEventContext,
        ) -> Result<GcReport> {
