...
pub async fn analyze_region_migration_task(
    ...
    for (table_id, table_route) in table_ids.into_iter().zip(table_routes) {
        ...
        for region_route in region_routes {
            if region_ids.contains(&region_route.region.id) {
                ...
                // For regions in the task but not present in current table routes,
                // they might have been removed after repartition. Mark them as region_not_found
                // so that upper layers can stop retrying failover on them.
                result.region_not_found.push(region_route.region.id);
            }
        }
    }
