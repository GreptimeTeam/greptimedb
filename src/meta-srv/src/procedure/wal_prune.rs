...
use common_meta::instruction::{FlushRegion, Instruction};
...
impl WalPruneProcedure {
    ...
    fn build_flush_region_instruction(
        ...
        let peer_and_instructions = peer_region_ids_map
            .into_iter()
            .map(|(peer, region_ids)| {
                let flush_instruction = Instruction::FlushRegion(FlushRegion {
                    region_ids,
                    is_hint: true,
                    fail_fast: false,
                });
...
mod tests {
    ...
    async fn check_flush_request(
        ...
        let flush_requested_region_ids = match flush_instruction {
            Instruction::FlushRegion(FlushRegion { region_ids, .. }) => region_ids,
            _ => unreachable!(),
        };
