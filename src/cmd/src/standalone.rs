...
impl StartCommand {
    ...
    async fn build(&self, opts: GreptimeOptions<StandaloneOptions>) -> Result<Instance> {
        ...
        plugins::setup_frontend_plugins(&mut plugins, &fe_opts, self.config_file.as_ref())
            .await
            .context(StartFrontendSnafu)?;

        plugins::setup_datanode_plugins(&mut plugins, &dn_opts, self.config_file.as_ref())
            .await
...
mod tests {
    ...
    async fn test_try_from_start_command_to_anymap() {
        ...
        let mut plugins = Plugins::new();
        plugins::setup_frontend_plugins(&mut plugins, &fe_opts, None)
            .await
            .unwrap();
