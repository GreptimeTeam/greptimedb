...
pub(super) async fn run_direct_sst(args: DirectArgs) {
    let case_path = args.case.as_ref().expect("--case is required");
    let out_dir = args.out_dir.as_ref().expect("--out-dir is required");
    let case_text = fs::read_to_string(case_path).expect("failed to read query perf case TOML");
    let case: CaseFile = toml::from_str(&case_text).expect("failed to parse query perf case TOML");
    let case_name = case_name_from_path(case_path);
