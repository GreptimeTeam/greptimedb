...
impl SeriesAggregateStream {
    ...
    fn aggregate_rate(&mut self, batch: &RecordBatch, rate: &FusedRateInput) -> Result<()> {
        ...
        let grid = self.grid;
        // Millisecond samples with no offset, which `fused_rate_source` required: the input
        // timestamps are already the milliseconds the range fold would have produced.
        let (ranges, (start, end)) = calculate_ranges(
            timestamps.values(),
            NANOS_PER_MILLI,
            0,
            grid.start,
