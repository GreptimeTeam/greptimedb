...
impl SortField {
    ...
    /// Checks boundaries before calling memcomparable's unchecked reads/advances.
    fn encoded_length(data_type: &ConcreteDataType, bytes: &[u8]) -> Result<usize> {
        ...
        if data_type.is_boolean() && bytes[1] > 1 {
            return Err(memcomparable::Error::InvalidBoolEncoding(bytes[1]))
                .context(error::DeserializeFieldSnafu);
        }
        Ok(to_skip)
    }
