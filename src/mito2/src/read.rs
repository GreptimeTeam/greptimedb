...
mod tests {
    ...
    async fn print_metadata() {
        let path = "/home/wayne/repo/greptimedb/greptimedb_data/data/greptime/nodeexporter/1024/1024_0000000000/data/91d141ac-2cfc-4685-ba42-0b00a6de9fcf.parquet";
        ...
        println!(
            "file: {path}, num_rows: {}, num_row_groups: {}",
            metadata.file_metadata().num_rows(),
            metadata.row_groups().len()
        );

        let mut op_type_uncompressed: i64 = 0;
        let mut op_type_compressed: i64 = 0;

        for (rg_idx, row_group) in metadata.row_groups().iter().enumerate() {
            println!(
                "row_group #{rg_idx}: rows={}, total_byte_size={}",
                row_group.num_rows(),
            ...
            for column in row_group.columns() {
                ...
                //     Some(stats) => println!("  column '{column_name}' stats: {stats:?}"),
                //     None => println!("  column '{column_name}' stats: <none>"),
                // }

                if column_name == "__op_type" {
                    op_type_uncompressed += column.uncompressed_size();
                    op_type_compressed += column.compressed_size();
                    println!(
                        "    __op_type => uncompressed: {}, compressed: {}",
        ...
        println!(
            "__op_type size => uncompressed(after encoding): {op_type_uncompressed} bytes, compressed: {op_type_compressed} bytes"
        );
    }
}
