...
impl PartToMerge {
    ...
    fn create_iterator(
        ...
        Ok(Some(Box::new(iter.map(move |batch| {
            batch.and_then(|batch| rewrite_json2_batch(batch, &targets))
        }))))
    }
}
...
impl MemtableCompactor {
    ...
    fn merge_parts_group(
        ...
        let source_schemas = parts_to_merge
            .iter()
            .map(|part| ((*part.arrow_schema()).clone(), part.num_rows() as u64))
            .collect::<Vec<_>>();
        let targets = Arc::new(json2_targets_from_schemas(metadata, &source_schemas)?);
        let schema = rewrite_json2_schema(&parts_to_merge[0].arrow_schema(), &targets);

        let iterators: Vec<BoxedRecordBatchIterator> = parts_to_merge
            .into_iter()
            .map(|part| part.create_iterator(context.clone(), targets.clone()))
...
mod tests {
    ...
    fn test_bulk_memtable_merge_bounds_json2_paths() -> WhateverResult<()> {
        let metadata =
            mock_metadata_with_json2(JsonSettings::default().with_max_auto_expanded_paths(1));
