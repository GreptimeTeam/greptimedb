...
impl Compactor for DefaultCompactor {
    async fn merge_ssts(
        ...
        for output in picker_output.outputs.drain(..) {
            ...
            let manifest_ctx_clone = manifest_ctx.clone();
            let compaction_time_window_clone = compaction_time_window;
...
mod tests {
    ...
    use store_api::region_request::{PathType, RegionCloseRequest, RegionRequest};
    ...
    /// Creates a simple CompactionRegion for testing
    async fn create_test_compaction_region() -> CompactionRegion {
        ...
        <dyn RegionEngine>::handle_request(&engine, region_id, RegionRequest::Create(request))
            .await
            .unwrap();

        <dyn RegionEngine>::handle_request(
            &engine,
            region_id,
    ...
    /// Creates a simple PickerOutput with expired SSTs
    fn create_picker_output_with_expired(
        expired_file_ids: Vec<FileId>,
        merge_outputs: Vec<CompactionOutput>,
    ) -> PickerOutput {
        let expired_ssts = expired_file_ids
            .into_iter()
            .map(|file_id| new_file_handle(file_id, 0, 1000, 0))
            .collect();
    ...
    #[tokio::test]
    async fn test_merge_ssts_expired_ssts_deleted_first() {
        let compaction_region = create_test_compaction_region().await;

        // Create expired SSTs
        let expired_file_id1 = FileId::random();
        let expired_file_id2 = FileId::random();

        let picker_output = create_picker_output_with_expired(
            vec![expired_file_id1, expired_file_id2],
            vec![], // No merge outputs
        );

        // Get initial manifest version
        let initial_version = compaction_region.manifest_ctx.manifest_version().await;
        ...
        // Verify expired SSTs are in the result
        assert_eq!(2, result.files_to_remove.len());
        assert_eq!(0, result.files_to_add.len());

        // Verify manifest was updated (version should increase)
        let final_version = compaction_region.manifest_ctx.manifest_version().await;
        assert!(
            final_version > initial_version,
            "Manifest should be updated when expired SSTs are removed"
        );

        // Verify expired files are in the result
        let removed_file_ids: Vec<FileId> =
            result.files_to_remove.iter().map(|f| f.file_id).collect();
        assert!(removed_file_ids.contains(&expired_file_id1));
        assert!(removed_file_ids.contains(&expired_file_id2));
    }

    #[tokio::test]
    async fn test_merge_ssts_no_expired_ssts() {
        let compaction_region = create_test_compaction_region().await;

        // Create picker output without expired SSTs
        ...
        assert_eq!(0, result.files_to_remove.len());
        assert_eq!(0, result.files_to_add.len());
    }

    #[tokio::test]
    async fn test_merge_ssts_expired_before_merge() {
        let compaction_region = create_test_compaction_region().await;

        // Create expired SSTs
        let expired_file_id = FileId::random();

        // Note: We can't easily test merge tasks updating manifest immediately
        // without more complex setup, but we can verify expired SSTs are handled first
        let picker_output = create_picker_output_with_expired(
            vec![expired_file_id],
            vec![], // Empty merge outputs for simplicity
        );
        ...
        // Verify expired file is in the result
        assert_eq!(1, result.files_to_remove.len());
        assert_eq!(expired_file_id, result.files_to_remove[0].file_id);
    }

    #[tokio::test]
    async fn test_merge_ssts_manifest_updates_ordering() {
        // This test verifies that expired SSTs are handled first by checking
        // that the manifest is updated before merge tasks start
        // (We can't easily verify individual task manifest updates without
        // more complex mocking, but we can verify the overall flow)

        let compaction_region = create_test_compaction_region().await;

        let expired_file_id = FileId::random();
        let picker_output = create_picker_output_with_expired(vec![expired_file_id], vec![]);
    ...
    #[tokio::test]
    async fn test_merge_ssts_expired_with_file_purger() {
        // Test that code works when file_purger is present
        let compaction_region = create_test_compaction_region().await;

        // Verify file purger exists
        assert!(compaction_region.file_purger().is_some());
        ...
        // Should complete successfully with file purger
        assert_eq!(1, result.files_to_remove.len());
        assert_eq!(expired_file_id, result.files_to_remove[0].file_id);
    }
}
