...
async fn test_json_type_hint_pushdown_scanner_returns_batches() -> WhateverResult<()> {
    ...
    assert_eq!(
        ...
        ReadColumns::new(
            [1, 0],
            BTreeMap::from([(1, ConcreteDataType::json2(JsonNativeType::Variant))])
        )
    );
...
"#;
    ...
    // JSON type hints are derived from the whole query plan and may include
    // columns from other table scans. Unknown hints must not fail this scan.
    let request = ScanRequest {
        projection: Some(vec![0]),
