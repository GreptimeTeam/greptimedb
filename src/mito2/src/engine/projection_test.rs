...
async fn test_scan_projection_with_format(flat_format: bool) {
    ...
    let projection_input = ProjectionInput::new().with_projection(vec![1, 3, 4]);
...
async fn test_scan_projection_without_primary_key_with_format(flat_format: bool) {
    ...
    let projection_input = ProjectionInput::new().with_projection(vec![0, 1]); // field_0 and field_1 (not ts)
    let request = ScanRequest {
        projection_input: Some(projection_input),
