...
async fn edit_region(
    ...
    if let Some(write_cache) = cache_manager.write_cache() {
        for file_meta in &edit.files_to_add {
            ...
            let remote_path = location::sst_file_path(layer.table_dir(), file_meta.file_id(), layer.path_type());
            ...
            let index_remote_path =
                location::index_file_path(layer.table_dir(), file_meta.file_id(), layer.path_type());
