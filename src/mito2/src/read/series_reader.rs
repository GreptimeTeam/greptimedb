...
mod tests {
    ...
    fn exact_sequence_filter_precedes_shared_dedup_for_tombstones_and_last_non_null() {
        ...
        use crate::read::flat_dedup::{FlatDedupIterator, FlatLastNonNull};

        let schema = Arc::new(Schema::new(vec![
            Field::new("tag_0", DataType::Utf8, false),
            Field::new("field_0", DataType::Int64, true),
        ...
        let batch = |timestamp: i64,
                     fields: &[(Option<i64>, Option<i64>)],
                     sequences: &[u64],
                     ops: &[OpType]| {
            let mut keys = BinaryDictionaryBuilder::<UInt32Type>::new();
            for _ in fields {
                keys.append(b"series").unwrap();
            }
            RecordBatch::try_new(
                schema.clone(),
                vec![
                    Arc::new(StringArray::from_iter_values(
                        (0..fields.len()).map(|_| "series"),
                    )) as ArrayRef,
                    Arc::new(Int64Array::from_iter(fields.iter().map(|field| field.0))) as ArrayRef,
                    Arc::new(Int64Array::from_iter(fields.iter().map(|field| field.1))) as ArrayRef,
                    Arc::new(TimestampMillisecondArray::from_iter_values(
                        (0..fields.len()).map(|_| timestamp),
                    )) as ArrayRef,
                    Arc::new(keys.finish()) as ArrayRef,
                    Arc::new(UInt64Array::from_iter_values(sequences.iter().copied())) as ArrayRef,
                    Arc::new(UInt8Array::from_iter_values(ops.iter().map(|op| *op as u8)))
                        as ArrayRef,
                ],
            )
            .unwrap()
        ...
        // The in-range tombstone reaches shared dedup and suppresses its put.
        let output = FlatDedupIterator::new(
            // Shared merge presents duplicate rows newest-first: the in-range
            // tombstone (seq 3) must therefore suppress the eligible put
            // (seq 2).
            vec![
                Ok(filter(batch(
                    1000,
        ...
        // A tombstone outside H is removed before shared dedup and cannot
        // suppress the eligible put.
        let output = FlatDedupIterator::new(
            vec![Ok(filter(batch(
                1000,
        ...
        // An outside-range non-null value is removed before LastNonNull, so it
        // cannot fill the eligible row's null field.
        let output = FlatDedupIterator::new(
            vec![Ok(filter(batch(
                1000,
                &[(Some(10), None), (None, Some(40))],
                &[2, 4],
