...
use crate::read::read_columns::JsonReadTargets;
use crate::read::{
...
impl FlatRowGroupLastRowCachedReader {
    pub(crate) fn new(
        ...
        if let Some(value) = cache_strategy.get_selector_result(&key) {
            ...
            let json_read_targets_match = value.json_read_targets == json_read_targets;
            if is_flat && schema_matches && json_read_targets_match {
                Self::new_hit(value)
            } else {
                Self::new_miss(key, read_cols, json_read_targets, reader, cache_strategy)
            }
        } else {
            Self::new_miss(key, read_cols, json_read_targets, reader, cache_strategy)
        }
