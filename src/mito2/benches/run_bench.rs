...
use mito2::compaction::run::{find_sorted_runs, reduce_runs, Item, Ranged};
...
fn bench_reduce_runs(c: &mut Criterion) {
    ...
    for size in [10, 100].iter() {
        group.bench_function(format!("size_{}", size), |b| {
            ...
            let required_num_runs = *size/2;
            b.iter(|| {
                reduce_runs(black_box(runs.clone()), black_box(required_num_runs));
            });
        });
    }
    group.finish();
...
criterion_group!(benches, bench_find_sorted_runs, bench_reduce_runs);
