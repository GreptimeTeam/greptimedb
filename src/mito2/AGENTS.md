...
| `series_index` | `src/mito2/src/series_index/` | Series index writer/searcher, bucket planning, catalogs, worker-owned maintenance and purge tasks, shared disk reservations (`disk_budget.rs`), startup trimming (`recovery.rs`), and index lifecycle |
...
- **Series-index disk budget**: all local index output and cleanup must use the shared budget;
  snapshots pin files until readers finish, and only physical deletion refunds permits.
