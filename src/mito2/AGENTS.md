...
| `series_index` | `src/mito2/src/series_index/` | Series index writer/searcher, bucket planning, catalogs, worker-owned maintenance and shared purge tasks, shared reconciliation and quota accounting (`maintenance.rs`), startup trimming (`recovery.rs`), and index lifecycle |
...
- **Series-index disk quota**: reconciliation charges installed files using catalog sizes and
  retires oldest minimum timestamps first. Unpublication releases usage; file handles delay
  physical deletion independently. Writers and catalog output do not reserve quota.
