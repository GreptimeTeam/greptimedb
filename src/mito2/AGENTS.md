...
| `series_index` | `src/mito2/src/series_index/` | Series index writer/searcher, bucket planning, catalogs, worker-owned maintenance and purge tasks, shared installed-file accounting (`disk_budget.rs`), startup trimming (`recovery.rs`), and index lifecycle |
...
- **Series-index disk quota**: reconciliation charges installed files using catalog sizes and
  retires oldest minimum timestamps first. Unpublication releases usage; reader pins delay
