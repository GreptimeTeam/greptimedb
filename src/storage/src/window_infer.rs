...
impl WindowInfer for PlainWindowInference {
    fn infer_window(
        ...
        let window_size = min_duration_to_window_size(min_duration_sec, max_durations_sec);
...
fn min_duration_to_window_size(min_duration: i64, max_duration: i64) -> i64 {
...
mod tests {
    ...
    fn test_get_time_window_size() {
        assert_eq!(1, min_duration_to_window_size(0, 0));
        for window in TIME_WINDOW_SIZE {
            assert_eq!(window, min_duration_to_window_size(window, window));
        }
        assert_eq!(1, min_duration_to_window_size(1, 1));
        assert_eq!(60, min_duration_to_window_size(60, 60));
        assert_eq!(60 * 10, min_duration_to_window_size(100, 100));
        assert_eq!(60 * 30, min_duration_to_window_size(1800, 1800));
        assert_eq!(60 * 60, min_duration_to_window_size(3000, 3000));
        assert_eq!(2 * 60 * 60, min_duration_to_window_size(4000, 4000));
        assert_eq!(6 * 60 * 60, min_duration_to_window_size(21599, 21599));
        assert_eq!(12 * 60 * 60, min_duration_to_window_size(21601, 21601));
        assert_eq!(24 * 60 * 60, min_duration_to_window_size(43201, 43201));
        assert_eq!(
            7 * 24 * 60 * 60,
            min_duration_to_window_size(604799, 604799)
        );
        assert_eq!(
            7 * 24 * 60 * 60,
            min_duration_to_window_size(31535999, 31535999)
        );
        assert_eq!(
            7 * 24 * 60 * 60,
            min_duration_to_window_size(i64::MAX, i64::MAX)
        );
    }

    #[test]
    fn test_get_time_window_size_with_diff() {
        assert_eq!(600, min_duration_to_window_size(60, 10000));
        assert_eq!(
            TIME_WINDOW_SIZE.last().copied().unwrap(),
            min_duration_to_window_size(60, i64::MAX)
        );
    }
