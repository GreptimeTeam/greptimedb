...
mod tests {
    ...
    use crate::test_util::{self, read_util, schema_util};

    #[test]
    fn test_projection() {
        // Build a region schema with 2 value columns. So the final user schema is
        // (k0, timestamp, v0, v1)
        let region_schema = test_util::schema_util::new_region_schema(0, 2);
    ...
    fn test_projected_schema_with_projection() {
        ...
        let region_schema = Arc::new(test_util::schema_util::new_region_schema(123, 3));
        ...
        let expect_schema = test_util::schema_util::new_region_schema(123, 1);
        assert_eq!(
