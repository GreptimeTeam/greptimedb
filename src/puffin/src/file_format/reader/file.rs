...
impl<R: io::Read + io::Seek> PuffinParser<R> {
    fn check_magic_sync(&mut self) -> Result<()> {
        let mut magic = [0; 4];
...
impl<R: AsyncRead + AsyncSeek + Unpin> PuffinParser<R> {
    async fn check_magic_async(&mut self) -> Result<()> {
        let mut magic = [0; 4];
        self.source
