...
pub(super) fn parameter_to_string(portal: &Portal<SqlPlan>, idx: usize) -> PgWireResult<String> {
    ...
    match param_type {
        &Type::VARCHAR | &Type::TEXT => {
            let param = portal.parameter::<String>(idx, param_type)?;
            let data = param.as_deref().unwrap_or("");
            Ok(format!("'{}'", unquote(data)))
        }
        &Type::BOOL => Ok(portal
    ...
    for idx in 0..param_count {
        ...
        let value = match &client_type {
            &Type::VARCHAR | &Type::TEXT => {
                ...
                let unquoted_data = data.map(|d| unquote(&d));
                if let Some(server_type) = &server_type {
                    match server_type {
                        ConcreteDataType::String(t) => {
                            if t.is_large() {
                                ScalarValue::LargeUtf8(unquoted_data)
                            } else {
                                ScalarValue::Utf8(unquoted_data)
                            }
                        }
                        _ => {
                            return Err(invalid_parameter_error(
