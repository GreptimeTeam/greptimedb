...
impl QueryTraceParams {
    fn from_jaeger_query_params(query_params: JaegerQueryParams) -> Result<Self> {
        ...
        internal_query_params.db = query_params.db.unwrap_or("public".to_string());
...
fn normalize_span_kind(span_kind: &str) -> String {
    ...
    if span_kind.starts_with(SPAN_KIND_PREFIX) {
        span_kind[SPAN_KIND_PREFIX.len()..].to_lowercase()
    } else {
