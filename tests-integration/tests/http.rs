     ...
     ($($service:ident),*) => {
        $(
            http_test!(
                ...
                test_prometheus_metric_name_union,
                test_prom_http_api,
                test_config_api,
                test_dashboard_api,
...
pub async fn test_prometheus_metric_name_union(store_type: StorageType) {
    ...
    let range_query = |query: &str| {
        client.get(&format!(
            "/v1/prometheus/api/v1/query_range?query={}&start=0&end=600&step=600",
            encode(query)
        ))
    };
    ...
    // Sample values arrive as their Prometheus wire strings, so the expectation is built from
    // JSON just like the response is.
    assert_eq!(
    ...
    assert_eq!(
        data.result,
        serde_json::from_value::<PromQueryResult>(json!([
            {"metric": {"__name__": "union_b", "idc": "i1"}, "values": [[0.0, "5.0"]]}
        ]))
        .unwrap()
    );
