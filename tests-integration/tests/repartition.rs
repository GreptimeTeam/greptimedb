...
async fn assert_append_count(
    ...
    let output = run_sql(
        instance,
        &format!("SELECT ts, device_id FROM {table} ORDER BY ts, device_id"),
        QueryContext::arc(),
    )
    .await
    .unwrap();
    let batches = match output.data {
        ...
        common_query::OutputData::RecordBatches(batches) => batches,
        _ => panic!("expected row output for {table}"),
    };
    let actual = batches
        .iter()
        .flat_map(|batch| {
            ...
            let device_ids = batch
                ...
                .unwrap();
            (0..batch.num_rows())
                .map(move |index| (timestamps.value(index), device_ids.value(index)))
        })
        .collect::<Vec<_>>();
    let expected_rows = (0..rows as i64)
        .map(|value| (value, value as i32))
        .collect::<Vec<_>>();
    assert_eq!(actual, expected_rows, "ordered rows for {table}");
